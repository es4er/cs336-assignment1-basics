...
![Final validation loss plotted against peak learning rate on a log axis, tracing a U shape](../wandb/lr_sweep_u_shape.png)

*Figure 1. Final validation loss against peak learning rate: the 11 cosine runs
trace a U in log-rate space with the sampled optimum at `2e-3`, while the
constant-`3e-3` control sits off the curve because it changes the schedule rather
than the peak.*
...
*Figure 2. Validation loss over the full 500-update budget, evaluated every 50
updates. `1e-1` stays stranded near 4.0 while the fine-screen rates bunch below
2.4, and every curve remains finite through step 500.*
...
*Figure 3. Training loss on the same evaluation cadence; it tracks validation loss
at every learning rate, so the ordering is set by optimization rather than by
overfitting.*
...
*Figure 4. Per-step training loss, one point per optimizer update: the high-rate
runs oscillate violently through warm-up and the ordering settles only after it
ends.*
