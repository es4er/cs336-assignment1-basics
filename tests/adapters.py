...
def _copy_reference_weights(module: torch.nn.Module, weights: dict[str, Tensor]) -> None:
    """Copy assignment-reference weights into modules with `(in, out)` Linear storage."""
    linear_suffixes = (
        "q_proj.weight",
        "k_proj.weight",
        "v_proj.weight",
    ...
    converted = {
        key: value.T if key.endswith(linear_suffixes) else value
        for key, value in weights.items()
    }
    own.update(converted)
    module.load_state_dict(own, strict=True)
...
def run_swiglu(
    ...
    with torch.no_grad():
        layer.w1.weight.copy_(w1_weight.T)
        layer.w2.weight.copy_(w2_weight.T)
        layer.w3.weight.copy_(w3_weight.T)
    return layer(in_features)
...
def run_multihead_self_attention(
    ...
    """
    layer = CausalSelfAttention(d_model, num_heads, device=in_features.device, dtype=in_features.dtype)
    with torch.no_grad():
        layer.q_proj.weight.copy_(q_proj_weight.T)
        layer.k_proj.weight.copy_(k_proj_weight.T)
        layer.v_proj.weight.copy_(v_proj_weight.T)
        layer.output_proj.weight.copy_(o_proj_weight.T)
    return layer(in_features)
...
def run_transformer_block(
    ...
    _copy_reference_weights(block, weights)
...
def run_transformer_lm(
    ...
    _copy_reference_weights(model, weights)
...
def run_rmsnorm(
    ...
    """
    layer = RMSNorm(d_model, eps=eps, device=in_features.device, dtype=in_features.dtype)
    with torch.no_grad():
        layer.weight.copy_(weights)
    return layer(in_features)
